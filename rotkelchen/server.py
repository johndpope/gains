...
from errors import InputError, AuthenticationError, PermissionError
from args import app_args
from rotkelchen import Rotkelchen
from utils import pretty_json_dumps
from inquirer import get_fiat_usd_exchange_rates
...
class RotkelchenServer(object):
    ...
    def set_main_currency(self, currency_text):
        self.set_main_currency(currency_text)
    ...
    def remove_exchange(self, name):
        result, message = self.remove_exchange(name)
        return {'result': result, 'message': message}

    def setup_exchange(self, name, api_key, api_secret):
        result, message = self.setup_exchange(name, api_key, api_secret)
        return {'result': result, 'message': message}

    def query_otctrades(self):
        trades = self.data.get_external_trades()
        return process_result(trades)

    def add_otctrade(self, data):
        result, message = self.data.add_external_trade(data)
        return {'result': result, 'message': message}

    def edit_otctrade(self, data):
        result, message = self.data.edit_external_trade(data)
        return {'result': result, 'message': message}

    def delete_otctrade(self, data):
        result, message = self.data.delete_external_trade(data)
        return {'result': result, 'message': message}

    def set_premium_credentials(self, api_key, api_secret):
        result, empty_or_error = self.set_premium_credentials(api_key, api_secret)
        return {'result': result, 'message': empty_or_error}

    def set_premium_option_sync(self, should_sync):
        self.data.db.update_premium_sync(should_sync)
        return True
    ...
    def query_blockchain_total(self):
        balances = self.blockchain.query_balances()['totals']
    ...
    def query_blockchain_balances(self):
        balances = self.blockchain.query_balances()
    ...
    def query_fiat_total(self):
        balances = self.query_fiat_balances()
        res = {'total': self.get_total_in_usd(balances)}
        return process_result(res)

    def query_fiat_balances(self):
        res = self.query_fiat_balances()
        return process_result(res)

    def set_fiat_balance(self, currency, balance):
        result, message = self.data.set_fiat_balance(currency, balance)
        return {'result': result, 'message': message}
    ...
    def query_trade_history(self, location, start_ts, end_ts):
        ...
        end_ts = int(end_ts)
        if location == 'all':
            return self.trades_historian.get_history(start_ts, end_ts)
    ...
    def query_asset_price(self, from_asset, to_asset, timestamp):
        price = self.data.accountant.query_historical_price(
    ...
    def process_trade_history(self, start_ts, end_ts):
        start_ts = int(start_ts)
        end_ts = int(end_ts)
        result = self.process_history(start_ts, end_ts)
        return process_result(result)
    ...
    def query_balances(self, save_data=False):
        ...
        result = self.query_balances(save_data)
    ...
    def get_eth_tokens(self):
        result = {'all_eth_tokens': self.data.eth_tokens, 'owned_eth_tokens': self.blockchain.eth_tokens}
        return process_result(result)
    ...
    def unlock_user(self, user, password, create_new, sync_approval, api_key, api_secret):
        ...
        try:
            self.unlock_user(user, password, create_new, sync_approval, api_key, api_secret)
            res['exchanges'] = self.connected_exchanges
            res['premium'] = True if hasattr(self.rotkelchen, 'premium') else False
            res['settings'] = self.data.db.get_settings()
